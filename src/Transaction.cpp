...
Transaction::Transaction(const std::string &id, const std::string &propertyId,
                         const std::string &clientId, double finalPrice,
                         const std::string &status, const std::string &notes)
    : id(id), propertyId(propertyId), clientId(clientId),
      finalPrice(finalPrice), status(status), notes(notes) {
  if (!validateId(id)) {
  ...
  if (status != Constants::TransactionStatus::PENDING &&
...
std::ostream &operator<<(std::ostream &os, const Transaction &trans) {
  os << "Transaction ID: " << trans.id << "\n"
     ...
     << "Date: " << trans.date << "\n"
     << "Final Price: " << std::fixed << std::setprecision(2)
     << trans.finalPrice << " руб.\n"
     << "Status: " << trans.status << "\n"
     << "Notes: " << trans.notes;
  return os;
}
...
std::string Transaction::toFileString() const {
  std::ostringstream oss;
  oss << id << "|" << propertyId << "|" << clientId << "|" << date << "|"
      << finalPrice << "|" << status << "|" << notes;
  return oss.str();
}
