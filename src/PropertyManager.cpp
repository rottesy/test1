...
void PropertyManager::addProperty(std::unique_ptr<Property> property) {
  ...
  if (findProperty(property->getId()) != nullptr) {
    throw PropertyManagerException("Property with ID " + property->getId() +
                                   " already exists");
  }
  properties.push_back(std::move(property));
}

void PropertyManager::addApartment(const std::string &id,
                                   const std::string &city,
                                   const std::string &street,
                                   const std::string &house, double price,
                                   double area, const std::string &description,
                                   int rooms, int floor, bool hasBalcony,
                                   bool hasElevator) {
  try {
    auto apartment = std::make_unique<Apartment>(
        id, city, street, house, price, area, description, rooms, floor,
        hasBalcony, hasElevator);
    addProperty(std::move(apartment));
  } catch (const std::exception &e) {
    throw PropertyManagerException(std::string("Failed to add apartment: ") +
                                   e.what());
  }
}
...
void PropertyManager::addHouse(const std::string &id, const std::string &city,
                               ...
                               const std::string &house, double price,
                               double area, const std::string &description,
                               int floors, int rooms, double landArea,
                               bool hasGarage, bool hasGarden) {
  try {
    auto houseObj = std::make_unique<House>(id, city, street, house, price,
                                            area, description, floors, rooms,
                                            landArea, hasGarage, hasGarden);
    addProperty(std::move(houseObj));
  } catch (const std::exception &e) {
    throw PropertyManagerException(std::string("Failed to add house: ") +
                                   e.what());
  }
}

void PropertyManager::addCommercialProperty(
    const std::string &id, const std::string &city, const std::string &street,
  ...
  try {
    auto commercial = std::make_unique<CommercialProperty>(
        id, city, street, house, price, area, description, businessType,
        hasParking, parkingSpaces, isVisibleFromStreet);
    addProperty(std::move(commercial));
  } catch (const std::exception &e) {
    throw PropertyManagerException(
        std::string("Failed to add commercial property: ") + e.what());
  }
}
...
std::vector<Property *>
PropertyManager::searchByPriceRange(double minPrice, double maxPrice) const {
  std::vector<Property *> result;
  for (const auto &prop : properties) {
...
PropertyManager::searchByAddress(const std::string &city,
  ...
  for (const auto &prop : properties) {
    ...
    if (!lowerStreet.empty() &&
        propStreet.find(lowerStreet) == std::string::npos) {
      matches = false;
    }
    if (!lowerHouse.empty() &&
        propHouse.find(lowerHouse) == std::string::npos) {
      matches = false;
    }

    if (matches &&
        (!lowerCity.empty() || !lowerStreet.empty() || !lowerHouse.empty())) {
      result.push_back(prop.get());
    }
  }
  return result;
  ...
  while (std::getline(file, line)) {
    ...
    try {
      if (type == "APARTMENT") {
        ...
        auto apartment = std::make_unique<Apartment>(
            id, city, street, house, price, area, desc, rooms, floor,
        ...
        auto houseObj = std::make_unique<House>(
            id, city, street, house, price, area, desc, floors, rooms, landArea,
            garage == 1, garden == 1);
        ...
        auto commercial = std::make_unique<CommercialProperty>(
            id, city, street, house, price, area, desc, businessType,
