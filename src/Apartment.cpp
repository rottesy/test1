...
Apartment::Apartment(const std::string &id, const std::string &city,
    ...
    : Property(id, city, street, house, price, area, description), rooms(rooms),
      floor(floor), hasBalcony(hasBalcony), hasElevator(hasElevator) {
...
std::string Apartment::toFileString() const {
  ...
  oss << "APARTMENT|" << id << "|" << city << "|" << street << "|" << house
      << "|" << price << "|" << area << "|" << description << "|"
      << (isAvailable ? "1" : "0") << "|" << rooms << "|" << floor << "|"
      << (hasBalcony ? "1" : "0") << "|" << (hasElevator ? "1" : "0");
  return oss.str();
}

Property *Apartment::clone() const {
  return new Apartment(id, city, street, house, price, area, description, rooms,
