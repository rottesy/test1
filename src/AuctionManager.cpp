...
void AuctionManager::addAuction(std::shared_ptr<Auction> auction) {
  ...
  if (findAuction(auction->getId()) != nullptr) {
    throw AuctionManagerException("Auction with ID " + auction->getId() +
                                  " already exists");
  }
...
std::vector<Auction *>
AuctionManager::getAuctionsByProperty(const std::string &propertyId) const {
  std::vector<Auction *> result;
  for (const auto &auction : auctions) {
...
void AuctionManager::saveToFile(const std::string &filename) const {
  ...
  for (const auto &auction : auctions) {
    ...
    for (const auto &bid : bids) {
      file << "BID|" << auction->getId() << FILE_DELIMITER
           << bid->toFileString() << "\n";
    }
  }
  file.close();
...
void AuctionManager::loadFromFile(const std::string &filename) {
  ...
  while (std::getline(file, line)) {
    ...
    if (line.length() >= BID_PREFIX_LENGTH &&
        line.substr(0, BID_PREFIX_LENGTH) == BID_PREFIX) {
      ...
      try {
        ...
        if (!std::getline(iss, amountStr, FILE_DELIMITER) ||
            amountStr.empty()) {
          continue;
        }
      ...
      std::string id, propertyId, propertyAddress, startingPriceStr,
          buyoutPriceStr, status, createdAt, completedAt;
      ...
      try {
        ...
        auto auction = std::make_shared<Auction>(
            id, propertyId, propertyAddress, startingPrice);
