...
Bid::Bid(const std::string &clientId, const std::string &clientName,
         double amount)
...
std::ostream &operator<<(std::ostream &os, const Bid &bid) {
  os << "Client: " << bid.clientName << " (ID: " << bid.clientId << ")\n"
     << "Amount: " << std::fixed << std::setprecision(PRICE_PRECISION)
     << bid.amount << " руб.\n"
     << "Time: " << bid.timestamp;
  return os;
}
...
std::string Bid::toFileString() const {
  std::ostringstream oss;
  oss << clientId << "|" << clientName << "|" << std::fixed
      << std::setprecision(PRICE_PRECISION) << amount << "|" << timestamp;
  return oss.str();
}
