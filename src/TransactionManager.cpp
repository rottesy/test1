...
TransactionManager::TransactionManager() : dataFilePath("data/transactions.txt") {
}
...
void TransactionManager::saveToFile(const std::string& filename) const {
    ...
    if (!file.is_open()) {
        throw TransactionManagerException("Cannot open file for writing: " + filename);
    }
