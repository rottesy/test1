...
void ClientManager::addClient(std::shared_ptr<Client> client) {
  ...
  if (findClient(client->getId()) != nullptr) {
    throw ClientManagerException("Client with ID " + client->getId() +
                                 " already exists");
  }
...
std::vector<Client *>
ClientManager::searchByName(const std::string &name) const {
  std::vector<Client *> result;
...
std::vector<Client *>
ClientManager::searchByPhone(const std::string &phone) const {
  std::vector<Client *> result;
  for (const auto &client : clients) {
...
void ClientManager::saveToFile(const std::string &filename) const {
  ...
  for (const auto &client : clients) {
    file << client->getId() << FILE_DELIMITER << client->getName()
         << FILE_DELIMITER << client->getPhone() << FILE_DELIMITER
         << client->getEmail() << FILE_DELIMITER
