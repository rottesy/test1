...
void TransactionDialog::setupUI() {
  ...
  connect(propertyCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, &TransactionDialog::onPropertyChanged);
  ...
  priceSpin->setToolTip(
      "Цена сделки (может отличаться от цены недвижимости)\nМинимум: 10 000 "
  ...
  differenceLabel->setStyleSheet(
      "color: #6a9; font-size: 9pt; padding-left: 5px;");
  differenceLabel->setToolTip(
      "Разница между ценой сделки и ценой недвижимости");
  ...
  connect(buttonBox, &QDialogButtonBox::accepted, this,
          &TransactionDialog::validateAndAccept);
...
void TransactionDialog::validateAndAccept() {
  ...
  if (price < 10000) {
    QMessageBox::warning(this, "Ошибка",
                         "Цена сделки не может быть меньше 10 000 рублей");
    return;
  }
  if (price > 1000000000) {
    QMessageBox::warning(this, "Ошибка",
                         "Цена сделки не может превышать 1 000 000 000 рублей");
    return;
  }
...
void TransactionDialog::updatePriceFromProperty() {
  ...
  QString propId =
      (separatorIndex > 0) ? propIdText.left(separatorIndex) : propIdText;
    ...
    if (std::abs(difference) < 0.01) {
      ...
      diffText = QString("Переплата: +%1 руб. (+%2%)")
                     .arg(QString::number(difference, 'f', 2),
                          QString::number(percentDiff, 'f', 1));
      differenceLabel->setStyleSheet(
          "color: #a66; font-size: 9pt; padding-left: 5px;");
    } else {
