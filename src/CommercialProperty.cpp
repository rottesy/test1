...
CommercialProperty::CommercialProperty(
  ...
  if (parkingSpaces < MIN_PARKING_SPACES ||
      parkingSpaces > MAX_PARKING_SPACES) {
...
std::string CommercialProperty::toFileString() const {
  ...
  oss << "COMMERCIAL|" << id << "|" << city << "|" << street << "|" << house
      << "|" << price << "|" << area << "|" << description << "|"
      << (isAvailable ? "1" : "0") << "|" << businessType << "|"
      << (hasParking ? "1" : "0") << "|" << parkingSpaces << "|"
      << (isVisibleFromStreet ? "1" : "0");
  return oss.str();
}

Property *CommercialProperty::clone() const {
  return new CommercialProperty(id, city, street, house, price, area,
                                description, businessType, hasParking,
